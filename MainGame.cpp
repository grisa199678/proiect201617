		...
		case SDL_QUIT: 
			...
			if (event.type == SDL_KEYDOWN && _key==false) {
				if (event.key.keysym.sym == SDLK_SPACE) {
					_keyPressed = 1;
					_key = true;
					if (_gameState == GameState::PRESS) _gameState = GameState::PLAY; else 
					if (_gameState == GameState::TRYAGAIN) _gameState = GameState::PLAY; else {
						if (_gameState == GameState::PLAY) {
							if (_complexity < 12) _complexity += 0.7;
							checkDots();
						}
					}
				}
...
void MainGame::checkDots() {
	...
	int dots = 0;
	while (prim != nullptr) {
		if (abs(prim->r - _circleRadius-2) < 8) {
...
void MainGame::pressStart() {
	TTF_Init();
	TTF_Font *Sans = TTF_OpenFont("sans.ttf", 20);
	SDL_Surface* surfaceMessage = TTF_RenderText_Solid(Sans, "Press SPACE and START!", _blackColor);
	SDL_Texture* Message = SDL_CreateTextureFromSurface(_renderer, surfaceMessage);
	SDL_Rect again;
	_score = 0;
	again.x = 40;
	...
	TTF_Quit();
	aacircleRGBA(_renderer, _circleX, _circleY, _circleRadius, 0, 0, 0, 255);
}
